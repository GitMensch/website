...
```sh
./get_appveyor_artifacts.sh --account=ACCOUNT --project=PROJECT --flat --path $TMPDIR --token=TOKEN
```
...
function get_appveyor_artifacts() {
    ...
    fetch_from_api() {
        local url="$1" response message

        response=$(curl "${curl_opts[@]}" "${headers[@]}" -s "$url")
        if [[ -z "$response" ]]; then
            echo "Response of the following curl command was empty:" 1>&2
            echo curl "${curl_opts[@]}" "${headers[@]}" -s "$url" 1>&2
            return 1
        fi

        [[ "$debug" -eq 1 ]] && printf "DBG: curl command used:\nDBG: curl %s %s -s %s\n" "${curl_opts[*]}" "${headers[*]}" "$url" 1>&2

        # Extract and output message, if any
        message=$(echo "$response" | jq -r '.message')
        if [[ -n "$message" && "$message" != "null" ]]; then
            echo "Note: API request provided the following message: $message" 1>&2
        fi
    ...
    if [[ -n "$commit" ]]; then
        ...
        response=$(fetch_from_api "$project_uri") || rc=$?
        if [[ $rc -ne 0 ]]; then
            if [[ -z "$response" ]]; then
                echo "Error: Failed to fetch project history." 1>&2
            fi
            return "$rc"
        fi
    ...
    if [[ $rc -ne 0 ]]; then
        if [[ -z "$response" ]]; then
            echo "Error: Failed to fetch project details." 1>&2
        fi
        return "$rc"
    fi
    ...
    if [[ -n "$job_name" ]]; then
        ...
        if [[ -z "$job_id" ]]; then
            echo "Error: Unable to find a job named $job_name."1 1>&2
            return 1
        fi
    else
    ...
    [[ "$debug" -eq 1 ]] && echo "DBG: job_id for last run of Appveyor CI: $job_id" 1>&2

    # Fetch artifacts
    artifacts_response=$(curl "${curl_opts[@]}" "${headers[@]}" -s "$api_url/buildjobs/$job_id/artifacts")
    ...
    echo "$artifacts" | while IFS= read -r artifact; do
        ...
        [[ "$debug" -eq 1 ]] && echo "DBG: Downloading $artifact_url to $local_artifact_path..." 1>&2
